...
namespace irods
{
    ...
    static void resolve_hostnames_using_hosts_config(std::vector<std::string>& _hostnames)
    {
        const auto& hosts_config = get_server_property<const nlohmann::json&>(HOSTS_CONFIG_JSON_OBJECT_KW);

        std::transform(std::begin(_hostnames), std::end(_hostnames), std::begin(_hostnames),
            [&hosts_config](std::string& _hostname) {
                return resolve_hostname(_hostname, hosts_config, hostname_resolution_scheme::match_preferred)
                    .value_or(_hostname);
            });
    }
        ...
        std::string&       _output)
    {
        ...
        try {
            time_out = get_server_property<int>(CFG_SERVER_CONTROL_PLANE_TIMEOUT);
            port = get_server_property<int>(_port_keyword);
            num_hash_rounds = get_server_property<int>(CFG_SERVER_CONTROL_PLANE_ENCRYPTION_NUM_HASH_ROUNDS_KW);
    ...
    static error kill_server(const std::string& _pid_prop)
    {
        ...
        std::string output;
        irods::error ret = get_script_output_single_line("python", "kill_pid.py", args, output);
        if (!ret.ok()) {
            return PASS(ret);
        }
    ...
    static int get_pid_age(pid_t _pid)
    {
        ...
        std::string pid_age;
        irods::error ret = get_script_output_single_line("python", "pid_age.py", args, pid_age);
        if (!ret.ok()) {
            irods::log(PASS(ret));
            return 0;
        }
        ...
        : port_prop_(_prop)
    {
        ...
        provider_hostname_ = boost::any_cast<const std::string&>(
            get_server_property<const std::vector<boost::any>&>(CFG_CATALOG_PROVIDER_HOSTS_KW)[0]
        );
    ...
    void server_control_executor::operator()()
    {
        ...
        buffer_crypt::array_t shared_secret;
        try {
            port = get_server_property<int>(port_prop_);
            num_hash_rounds = get_server_property<int>(CFG_SERVER_CONTROL_PLANE_ENCRYPTION_NUM_HASH_ROUNDS_KW);
            encryption_algorithm.reset(get_server_property<const std::string>(CFG_SERVER_CONTROL_PLANE_ENCRYPTION_ALGORITHM_KW));
            const auto& key = get_server_property<const std::string>(CFG_SERVER_CONTROL_PLANE_KEY);
            shared_secret.assign(key.begin(), key.end());
    ...
    error server_control_executor::process_operation(const zmq::message_t& _msg, std::string& _output)
    {
        ...
        resolve_hostnames_using_hosts_config(cmd_hosts);
